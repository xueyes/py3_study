...
class Login(object):
    ...
    def getProvinceID(self):
        html = self.session.get(url=self.login_url).text
        soup = BeautifulSoup(html, "lxml")
        print(soup)
        ProvinceID = soup.select('#hidProvinceID')
        print(ProvinceID)
        return ProvinceID
    ...
    def login(self, usename, password):
        ...
        #判断是否登录成功标识
        soup = BeautifulSoup(responsed_html, "lxml")
        self.is_logined(soup)
        ...
        MyOrderInfo_url = 'http://www.189.cn/dqmh/userCenter/myOrderInfoList.do?method=listMyOrderInfo_new&isVs=no'
        res = self.session.get(url=MyOrderInfo_url,headers=headers)
        print("个人资料的cookie获取成功!!!!",res)

        baseService_url = 'http://service.gz.189.cn'
        response_balance = self.session.get(url=baseService_url, headers=headers)
        print("获取index.php   cookie 获取成功!!!!",response_balance)

        return logined_url

    #判断是否登录成功
    def is_logined(self,soup):
        if soup.select('div.header.w.h.z333 span.w420.h30.fl.ff_a iframe'):
            print("登陆成功")
        else:
            print("账号或者密码或者验证码错误!!!!!!!")
            exit(-1)

        # base = 'http://www.189.cn'
        # url = base + src_loginurl
    ...
    #最近3个月的通话详单部分功能函数(减少代码冗余度)
    def get_part_CalldetailsInfo(self,QueryMonthlyInfo,cell_phone_verification_code):
        ...
        for querymonthly in QueryMonthly:
            ...
            params = {
                ...
                'QueryType': '1',
                # 'checkcode':'undefined',
                'checkcode': cell_phone_verification_code,
                '_': querytime
            }
    ...
    def get_CalldetailsInfo(self):
        ...
        base = 'http://service.gz.189.cn/web/query.php?'
        params_call = {
            'action':'call'
        }
        url = base + urlencode(params_call)
        print(url)
        response = self.session.get(url=url,headers=headers)
        html = response.text
        ...
        #判断是否需要发送验证码(第一次需要发送验证码)
        if soup.select('input#getcheckcode'):
            print(soup.select('input#getcheckcode'))
            params_postsms = {
                'action':'postsms'
            }
            checkcode_url = base + urlencode(params_postsms)
            print(checkcode_url)
            headers = {
            ...
            self.session.post(url = checkcode_url)
            cell_phone_verification_code = input("请输入手机验证码：")
            CalldetailsInfodict = self.get_part_CalldetailsInfo(QueryMonthlyInfo,cell_phone_verification_code)
            return CalldetailsInfodict
